...
# hàm thực hiện việc chuyển đổi nhiệt độ F sang C
# Sử dụng “\N{DEGREE CELSIUS}” để hiển thị biểu tượng ℃
...
# hàm thực hiện việc chuyển đổi nhiệt độ C sang F
# Sử dụng “\N{DEGREE FAHRENHEIT }” để hiển thị biểu tượng ℉
...
# phương thức grid() để sắp xếp bố cục
...
# Dùng widget Button() để tạo button thực hiện việc các gọi hàm chuyển đổi nhiệt độ F sang C
btn_convert_celsius = Button(
    master=window,
    text="\N{RIGHTWARDS BLACK ARROW}",
    command=celsius_to_fahrenheit
)
# Dùng widget Label() để khai báo vùng chứa trả về kết quả sau khi chuyển đổi nhiệt độ F sang C
